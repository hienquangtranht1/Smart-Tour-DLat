...
public class StaffController {
    ...
    @Autowired
    private com.dalat.hien.tqh.smarttour.service.EmailService emailService;
    ...
    public ResponseEntity<?> createService(
        ...
        if (name == null || name.trim().isEmpty()) {
            return ResponseEntity.badRequest().body("Tên dịch vụ không được để trống!");
        }
        if (price == null || price.compareTo(BigDecimal.ZERO) < 0) {
            return ResponseEntity.badRequest().body("Giá tiền không hợp lệ! (Phải lớn hơn hoặc bằng 0 lớn)");
        }
        if (description == null || description.trim().isEmpty()) {
            return ResponseEntity.badRequest().body("Mô tả dịch vụ không được để trống!");
        }
        if (maxPeople != null && maxPeople < 1) {
            return ResponseEntity.badRequest().body("Số lượng người tối đa phải lớn hơn 0!");
        }
